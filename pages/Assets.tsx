...
const Assets = () => {
  ...
  const [displayMode, setDisplayMode] = useState<'grid' | 'list'>(() => {
    return (localStorage.getItem('assetsViewMode') as 'grid' | 'list') || 'list';
  });

  useEffect(() => {
    localStorage.setItem('assetsViewMode', displayMode);
  }, [displayMode]);
    ...
    <div className="space-y-6 animate-fadeIn">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 md:mb-8 gap-4">
        ...
        <div className="flex items-center gap-3 self-start md:self-auto flex-wrap">
          ...
          <div className="bg-white border border-slate-200 rounded-lg px-4 py-2 text-sm font-medium text-slate-600 shadow-sm hidden sm:block">
          ...
          <div className="md:hidden space-y-4">
            {displayedAssets.map((asset, index) => (
              <AssetCard
                ...
                viewMode={viewMode}
                user={user}
                availableApps={[...availableApps, ...(user?.customApps || [])]}
                onEdit={(asset) => {
                  setOriginalAsset(asset);
